...
$result = mysqli_query($db, "SELECT * FROM inv_pcs WHERE id = $id");
if (mysqli_num_rows($result) > 0) {
    $row = mysqli_fetch_array($result);
    $fetchpcName = $row["pcName"];
    $fetchpcType = $row["pcType"];
    $fetchpcQuality = $row["pcQuality"];
    $fetchpcStatus = $row["pcStatus"];
}
...
if (isset($_POST["add-item"])) {
    ...
    if ($pcNameError == "" && $pcTypeError == "" && $pcQualityError == "" && $pcStatusError == "") {
        $query = mysqli_query($db, "UPDATE inv_pcs SET pcName = '$pcName',pcType = '$pcType',pcQuality = '$pcQuality',pcStatus = '$pcStatus' WHERE id = $id");
            ...
            <div class="navigation-area">
                <nav>
                    <ul>
                        ...
                        <a href="list-customer"> <!-- this page will have all the customers informations -->
                            <li>
                                All Customers
                            </li>
                        </a>
        ...
        <section>
            <div class="add-item-section">
                <form action="" method="post" class="add-item-form">
                    ...
                    <input type="text" name="pcName" placeholder="Computer Name" value="<?php echo $fetchpcName; ?>">
                    <span class="error-message">
                        <?php
                        echo $pcTypeError;
                        ?>
                    </span>
                    <input type="text" name="pcType" placeholder="Computer Type" value="<?php echo $fetchpcType; ?>">
                    <span class="error-message">
                        <?php
                        echo $pcQualityError;
                        ?>
                    </span>
                    <input type="text" name="pcQuality" placeholder="Computer Quality" value="<?php echo $fetchpcQuality; ?>">
                    <span class="error-message">
                        <?php
                        echo $pcStatusError;
                        ?>
                    </span>
                    <input type="text" name="pcStatus" placeholder="Computer Status" value="<?php echo $fetchpcStatus; ?>">
